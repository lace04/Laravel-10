			...
			<div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
				<div class="p-6 text-gray-900 dark:text-gray-100">
					<form method="POST" action="{{ route('chirps.store') }}">
						@csrf
						<textarea name="message" rows="3" class="block w-full rounded-md border-gray-300 bg-white shadow-sm transition-colors duration-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 dark:border-gray-600 dark:bg-gray-800 dark:text-white dark:focus:border-indigo-300 dark:focus:ring dark:focus:ring-indigo-200 dark:focus:ring-opacity-50" placeholder="{{ __('What\'s on your mind?') }}">{{ old('message') }}</textarea>
			...
			<div class="mt-6 bg-white dark:bg-gray-800 shadow-sm rounded-lg divide-y dark:divide-gray-900">
				...
				<div class="p-6 flex space-x-2">
					...
					<div class="flex-1">
						...
						<a href="#" class="mt-1 text-sm text-gray-600 dark:text-gray-400 hover:underline">{{ __('Reply') }}</a>
					</div>
					@can('update', $chirp)
					<x-dropdown>
						<x-slot name="trigger">
						...
						<x-slot name="content">
							...
							<form method="POST" action="{{route('chirps.destroy', $chirp)}}">
								@csrf @method('DELETE')
								<x-dropdown-link :href="route('chirps.destroy', $chirp)" class="text-red-600 hover:text-red-500" onclick="event.preventDefault(); if(confirm('{{ __('Are you sure?') }}')) this.closest('form').submit();">
									{{ __('Delete Chirp') }}
								</x-dropdown-link>
							</form>
